...
from pydoe_bk.doe_optimal.criterion import regularized_inv
from pydoe_bk.doe_optimal.utils import information_matrix
...
def a_efficiency(X: np.ndarray) -> float:
    ...
    M = information_matrix(X, normalized=True, alpha=0.0, X0=None)
    p = X.shape[1]
    return 100.0 * (p / np.trace(regularized_inv(M)))
